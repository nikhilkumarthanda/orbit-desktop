...
const api: OrbitAPI = {
  ...
  onAssistantState: callback => { const listener = (_event: Electron.IpcRendererEvent, state: Parameters<typeof callback>[0]) => callback(state); ipcRenderer.on("orbit:overlay:state", listener); return () => ipcRenderer.removeListener("orbit:overlay:state", listener); },
  ...
  onBrowserTask: callback => { const listener = (_event: Electron.IpcRendererEvent, payload: Parameters<typeof callback>[0]) => callback(payload); ipcRenderer.on("orbit:browser:task:event", listener); return () => ipcRenderer.removeListener("orbit:browser:task:event", listener); },
  onEmbeddedBrowserState: callback => { const listener = (_event: Electron.IpcRendererEvent, payload: Parameters<typeof callback>[0]) => callback(payload); ipcRenderer.on("orbit:embedded-browser:state", listener); return () => ipcRenderer.removeListener("orbit:embedded-browser:state", listener); },
