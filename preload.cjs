  ...
  const start = () => {
    ...
    if (browserTaskState?.status === "waiting_for_confirmation") {
      ...
      const activityLabels = {
        reading_dom: "READING DOM",
        visual_inspection: "VISUAL INSPECTION",
        target_found: "TARGET FOUND",
        acting: "ACTING",
        verifying: "VERIFYING",
      };
      const activityLabel = activityLabels[state.agentActivity] || "";
      if (activityLabel) agent.classList.add("visual");
      agentLabel.textContent = activityLabel || (browserTaskState?.planner === "native"
