...
export async function answerWithGemini(input: { query: string; history: ConversationTurn[]; sources?: ResearchSource[]; imageBase64?: string }) {
  ...
  const parts: Array<Record<string, unknown>> = [{ text: `You are Orbit, a natural, concise Mac assistant. Address the user as Boss only when it sounds natural, not in every reply. Handle casual conversation and long dictated requests conversationally. Lead with a short answer suitable for speech; add detail only when the request needs it. Answer only the user-facing final answer; never reveal analysis or <think> text. Use supplied evidence for current facts and cite it with [number]. If no evidence is supplied, answer from your knowledge and admit uncertainty when needed.\nConversation:\n${context}\nUser: ${input.query}${evidence}` }];
