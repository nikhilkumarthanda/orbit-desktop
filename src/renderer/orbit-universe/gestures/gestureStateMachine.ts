...
import { classify, pinchRatio, type GestureName } from "./classify";

export type Point = { x: number; y: number };
export type HandState = { landmarks: NormalizedLandmark[]; point: Point; palm: Point; gesture: GestureName; pinching: boolean };
...
export class GestureStabilizer {
  ...
  update(landmarks: NormalizedLandmark[], timestampMs: number): HandState {
    ...
    this.lastPalm = palm;
    return { landmarks, point, palm, gesture: this.committed, pinching: this.committed === "Pinch" || this.committed === "Drag" };
  }
}
